...
import { createApp, createFeature, generateProject } from "@loomstack/generator"
...
function writeFeature() {
  ...
  writeFileSync(join(base, "model.schema.ts"), `import { entity } from "@loomstack/runtime"\nexport const PersonSchema = entity("Person", {})`)
  writeFileSync(join(base, "actions/create.action.ts"), `import { action } from "@loomstack/runtime"\nexport const createPerson = action({})`)
  writeFileSync(join(base, "queries/list.query.ts"), `import { query } from "@loomstack/runtime"\nexport const listPeople = query({})`)
  writeFileSync(join(base, "ui/list.view.tsx"), `import { view } from "@loomstack/react"\nexport default view({ name: "PeopleListView", render() { return null } })`)
  generateProject(root)
}

beforeEach(() => {
  const parent = mkdtempSync(join(tmpdir(), "loomstack-verifier-"))
...
describe("verifier", () => {
  ...
  it("detects UI database access and raw fetch", () => {
    ...
    expect(verifyProject(root).errors.map((error) => error.code)).toEqual(expect.arrayContaining(["loomstack2001", "loomstack2002"]))
  })

  it("detects manually modified generated files", () => {
    writeFeature()
    appendFileSync(join(root, "apps/web/src/routes.generated.tsx"), "// manual edit\n")
    expect(verifyProject(root).errors).toEqual(expect.arrayContaining([expect.objectContaining({ code: "loomstack4001", file: "apps/web/src/routes.generated.tsx" })]))
  })
  ...
  it("distinguishes stale generated files after a contract change", () => {
    ...
    expect(verifyProject(root).errors).toEqual(expect.arrayContaining([expect.objectContaining({ code: "loomstack4002" })]))
  })
  ...
  it("reports manifest/implementation mismatches with stable codes", () => {
    ...
    expect(verifyProject(root, { generated: false }).errors.map((error) => error.code)).toEqual(expect.arrayContaining(["loomstack1003", "loomstack1004"]))
  })

  it("reports a missing project config", () => {
    const empty = mkdtempSync(join(tmpdir(), "loomstack-empty-"))
    expect(verifyProject(empty).errors[0]).toMatchObject({ code: "loomstack5001" })
  })
})
